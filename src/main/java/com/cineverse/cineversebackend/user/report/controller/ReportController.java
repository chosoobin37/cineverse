...
public class ReportController {
    ...
    public ResponseEntity<Report> processReportDeny (@RequestBody ReportDTO reportDTO, @PathVariable int boardReportId) {
        return ResponseEntity.ok(reportService.processReportDeny(boardReportId, reportDTO));
    }

    /* 설명. 게시글 신고 접수(승인O) */
    @GetMapping("/processAccept/{boardReportId}")
    public ResponseEntity<Report> processReportAccept (@RequestBody ReportDTO reportDTO, @PathVariable int boardReportId) {
        return ResponseEntity.ok(reportService.processReportAccept(boardReportId, reportDTO));
    }
