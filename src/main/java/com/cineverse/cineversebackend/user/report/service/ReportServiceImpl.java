...
public class ReportServiceImpl implements ReportService{
    ...
    @Override
    public void registReport(Report report) {
        Date date = new Date();
    ...
    /* 설명. 신고 반려 */
    @Override
    public Report processReportDeny(int boardReportId, ReportDTO reportDTO) {
    ...
    /* 설명. 신고 승인 */
    @Override
    public Report processReportAccept(int boardReportId, ReportDTO reportDTO) {
        Optional<Report> optionalReport = reportRepository.findById(boardReportId);
        if (!optionalReport.isPresent()) {
            throw new EntityNotFoundException("신고 내역이 존재하지 않습니다.");
        }
